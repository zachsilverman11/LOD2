...
export function LeadCard({ lead, onClick, isSelected }: LeadCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={`group bg-white p-4 rounded-xl shadow-sm border border-[#E5E0D8] cursor-pointer transition-all duration-150 hover:shadow-md hover:border-[#625FFF]/30 hover:-translate-y-0.5 hover:scale-[1.01] relative ${getLeftBorderClass()} ${
        isStale && !isSelected ? "bg-orange-50/30" : ""
      } ${isSelected ? "ring-2 ring-[#625FFF]/20 shadow-md" : ""}`}
    >
      {/* Drag Handle - appears on hover */}
      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <svg
          className="w-4 h-4 text-[#8E8983]"
          viewBox="0 0 24 24"
          fill="currentColor"
        >
      ...
      <div className="flex items-center justify-between mb-1">
        <h3 className="font-medium text-[#1C1B1A] text-sm truncate pr-6">
        ...
        <div className="relative group/holly">
          ...
          />
          {/* Tooltip */}
          <div className="absolute right-0 top-full mt-1 px-2 py-1 bg-[#1C1B1A] text-white text-[10px] rounded whitespace-nowrap opacity-0 group-hover/holly:opacity-100 transition-opacity pointer-events-none z-10">
      ...
      {/* Middle: Email */}
      <p className="text-xs text-[#8E8983] truncate mb-3">{lead.email}</p>

      {/* Bottom Row: Timestamp + Indicators */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className={`text-xs ${isStale ? "text-orange-600 font-medium" : "text-[#8E8983]"}`}>
            {getRelativeTime()}
          </span>
          {/* Stale indicator */}
          {isStale && (
