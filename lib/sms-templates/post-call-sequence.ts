...
export function buildDay5UrgencySms(params: PostCallSmsParams): string {
  const { clientFirstName } = params;
  return `Hi ${clientFirstName} — just a heads up, rates have been moving and your report numbers are based on today's market. The sooner you lock in, the better your options. Your secure application takes ~15 min: ${APPLICATION_URL}`;
}
...
export function buildDay10FinalSms(params: PostCallSmsParams): string {
  const { clientFirstName } = params;
  return `Hey ${clientFirstName} — this is my last check-in on your mortgage report. If the timing isn't right, totally understand. But if you're still thinking about it, your application link is here whenever you're ready: ${APPLICATION_URL} — Wishing you the best! 🏡`;
}
